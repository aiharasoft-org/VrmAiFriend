...
AI の応答は **Aivis Speech** で音声合成して再生します。会話からダンスを踊らせたり、ポーズさせたりできます。タイマーや音楽操作、バッテリー確認、天気予報・ニュース見出しなどの声かけにも対応します。会話のトーンに合わせて **表情が滑らかに変わり**、積み重ねで **好感度** も変化します。
...
AI が話している最中に話しかけると、再生を止めてあなたの発話を優先します。スピーカー利用時は **マイク感度を調整** すると、エコーによる誤検知を抑えつつ割り込みやすくなります。  
会話のつなぎ直しを待っているあいだは、キャラが待機ポーズをとることがあります。その最中にタッチすると、接続が切れていれば再接続を始めつつ待機ポーズをやめて反応し、終わったら通常の立ち姿とアイドルに戻ります。
...
スピーカーで聞く場合は **設定** タブの **マイク感度を調整** を行うと、AI の発話中でもエコーを抑えつつ、大きな声で割り込める。ヘッドホン利用時は調整不要なことが多い。物理マイク接続後は **設定** タブの **マイク** 状態が `OK` になることを確認してください。
...
「ブラウザ開いて」「メモ帳起動して」「電卓」「ファインダー／エクスプローラー」「ターミナル」などと話すと、許可されたアプリだけを起動できます。「LINE」「Slack」「Discord」で各アプリ（インストール済みの場合）、「GitHub」「ノートを開いて」「Yahooニュースを開いて」で各サイトを開けます。「アクティビティモニターを開いて」「タスクマネージャを開いて」「パフォーマンスメーターを開いて」と言うと、macOS ではアクティビティモニタ、Windows ではタスクマネージャを開きます。任意の URL や許可外のアプリは開けません。

### Web 検索

「〇〇について教えて」「〇〇を調べて」などと話すと、会話だけでは詳しく答えきれないときに Google 検索結果を既定ブラウザで開くことがあります。知識がある話題は会話だけで答えてよく、毎回検索する必要はありません。「ブラウザを開いて」は検索語句なしでブラウザを起動します。検索結果の本文をアプリ内で読んで答えることはしません。

### 天気予報

「相模原市の明日の天気」「東京の天気どう？」「横浜の一週間の天気」のように、地域名と日付をつけて聞くと、日本国内の予報を答えてくれます（今日・明日・明後日・週間など）。昨日など過去の天気は対象外です。天気の確認にブラウザを開く必要はありません。

### ニュース見出し

「今日のニュース」「国際ニュース教えて」「経済の見出し」などと話すと、カテゴリごとの最新見出しを教えてくれます（主要・国内・国際・経済・エンタメ・スポーツ・IT・地域）。「欧州の首脳会談の記事を詳しく」のように見出しの言葉で言うと、近い記事をブラウザで開けます。Yahoo!ニュースのトピックス一覧ページを開きたいときは「Yahooニュースを開いて」と言ってください。
...
| `F` キー               | —                   | —                | 視線追従を固定 / 解除                            |
| `P` キー               | —                   | —                | ポーズ撮影モードの開始 / 終了                    |
| ミュート               | `Option` + `M`      | `Alt` + `M`      | マイク送信のミュート切替（ミュート中は画面中央下に「Mute」）。「ミュートして」などでもミュート可。顔・胸タッチでも解除できる |
| ドラッグ               | —                   | —                | ウィンドウを移動                                 |
| クリック（頭・胸など） | —                   | —                | タッチリアクション（表情・音声・アニメーション）。顔・胸はミュート中なら解除も行う |
...
最新リリース（v0.9.6）の概要は [RELEASE_NOTES.md](RELEASE_NOTES.md) を参照してください。