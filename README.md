...
4. 準備に問題があると、短い音声で知らせる（Aivis 未起動、AI 未接続、マイクなし、VRoid Hub 復元失敗など）。問題がなければ、キャラクターが短い挨拶をする（言い回しは毎回変わる）。
...
| **会話**   | モード・AI 主導トーク・環境音ケア・話題・間隔の設定                              |
...
会話中に、ダンス名を話しかけるとキャラクターが踊ります。「ストップ」「やめて」「音楽停止」などと言うと、ダンス・ポーズ・再生中の音楽を止められます。
...
| 左右（オービット） | `Option` + `Z` / `C` | `Alt` + `Z` / `C`  | VRM 中心に右 / 左へ回る       |
| ズーム（微調整）   | `Option` + `W` / `S` | `Alt` + `W` / `S`  | 前進 / 後退                   |


カメラの高さ・チルト・左右シフト・オービット・ズームは次回起動時に復元される。**カメラ** タブの **カメラを初期値に戻す** で規定値に戻せる。

## 更新履歴

バージョンごとの変更点は [CHANGELOG.md](CHANGELOG.md) を参照してください。  
最新リリース（v0.9.0）の概要は [RELEASE_NOTES.md](RELEASE_NOTES.md) を参照してください。