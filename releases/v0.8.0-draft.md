...
**GitHub Releases 用（ドラフト）**

| 項目 | 値 |
| ---- | -- |
| Tag | `v0.8.0` |
| Title | `v0.8.0 — Windows 対応版` |
| Target | `main` |
| Draft | `true` |

---

<!-- 以下を GitHub Releases の Description に貼り付け -->
...
- **Windows CPU 向け Tips** — 電源モード「最適なパフォーマンス」の案内
...
> バイナリーはリリース Assets に手動でアップロードしてください。
...
| `OK` | 物理マイクから音声を取得中 |
| `準備完了` | 物理マイクあり・権限 OK |
| `物理マイクが見つかりません` | 仮想オーディオ（VB-Audio 等）のみ |
| `権限なし` | OS のマイク権限が未許可 |
...
- **Windows 版は新規対応**（本リリースの主題）。macOS 版の既存データは同じ形式で利用可能
- 話速は未設定時 **1.0**
- マイク感度を調整済みの場合、**マイク感度調整** のやり直しを推奨
- 仮想オーディオのみの環境では **物理マイクが見つかりません** と表示される
